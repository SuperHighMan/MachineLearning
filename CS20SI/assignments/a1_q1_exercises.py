...
# YOUR CODE
x1 = tf.random_normal([300], mean=300, stddev=1)
y1 = tf.random_normal([300], mean=300, stddev=1)
average = tf.reduce_mean(x1-y1)
def f1(): return tf.reduce_mean(tf.square(x1 - y1))
def f2(): return tf.reduce_sum(tf.abs(x1 - y1))
out_1h = tf.cond(tf.less(average, 0), f1, f2)
sess = tf.InteractiveSession()
